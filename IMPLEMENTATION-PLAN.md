...
| **5 — Test Coverage** | App Builder/Spring/Sling engines; **real coverage** (JaCoCo/PHPUnit-clover/nyc); `generateTests()` per framework (JUnit/MockMvc, PHPUnit/MFTF, Jest, Sling-Mocks); standardized outputs. |
| **6 — Generation** | Wire deterministic scaffolder (or formalize LLM path) to also emit CHANGE-LOG + report + branch; add App Builder/Spring/Sling to gen registry + resources. |
