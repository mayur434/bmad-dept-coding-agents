...
function writeComplianceIndex(
  ...
  lines.push("");
  lines.push(`_Generated by BMAD Compliance Agent — headless CLI fallback._`);
  ...
  lines.push("");
  lines.push(
    "This is the headless CLI fallback — it computes real control-mapping stats from cached findings but emits the artifact files as reporting plumbing only. For fully authored control-mapping matrices, audit-trail exports, cover letters, remediation plans, and attestation content, drive this agent conversationally from an AI coding tool: it reads `SKILL.md` and the `templates/`/`resources/` packs, then calls this same reporting layer.",
  );
  lines.push("");
