/**
 * BMAD Release Agent — Engine Registry
 * ====================================
 * Auto-detects platform engine or resolves explicit engine ID.
 * Each engine module exports a `main(input)` that produces release "findings"
 * (pipeline / release-notes / deploy-plan / rollback-plan / env-diff rows)
 * plus the list of files written.
 *
 * Content lands in Phase 3.2 — every engine here is a stub that returns
...
// ---------------------------------------------------------------------------
// Public release contract every engine implements
// ---------------------------------------------------------------------------

export interface ReleaseInput {
  projectRoot: string;
  pipeline: string;                 // resolved CI/CD target
  fromRef?: string;                 // git ref start
  toRef: string;                    // git ref end (default HEAD)
  env?: string;                     // source env for diff
  toEnv?: string;                   // target env for diff
  rollout: string;                  // deploy strategy
...
export interface ReleaseResult {
  findings: Finding[];              // pipeline / notes / plan / rollback / diff rows
  writtenFiles: string[];           // paths of files written
  stats: { pipelines: number; releaseNotes: number; plans: number; diffs: number };
}

export interface ReleaseEngine {
  id: string;
  name: string;
  main: (input: ReleaseInput) => Promise<ReleaseResult>;
}

// ---------------------------------------------------------------------------
// Engine imports (Phase 3.1 stubs — Phase 3.2 replaces bodies with real
// stack-specific release authoring logic).
// ---------------------------------------------------------------------------

import { main as aemMain } from "./aem/release";
import { main as commercePaasMain } from "./commerce-paas/release";
import { main as commerceSaasMain } from "./commerce-saas/release";
import { main as slingMain } from "./sling/release";
import { main as springMain } from "./spring/release";
import { main as appBuilderMain } from "./app-builder/release";
import { main as edsMain } from "./eds/release";
import { main as edsCommerceMain } from "./eds-commerce/release";
...
interface EngineEntry extends ReleaseEngine {
...
export function getEngine(engineId: string | null, projectPath: string): ReleaseEngine | null {
  if (engineId) {
...
export function listEngines(): void {
  console.log("Available release-authoring engines:");
  console.log("");
  for (const entry of ENGINES) {
