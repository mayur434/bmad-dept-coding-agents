...
function writeCodeReviewIndex(
  ...
  lines.push("");
  lines.push(`_Generated by BMAD Code Review Agent — headless CLI fallback._`);
  ...
  lines.push("");
  lines.push(
    "This is the headless CLI fallback — it detects real diff scope but emits reporting plumbing only. For real style-check / breaking-change / dependency-review / design-pattern / checklist content, drive this agent conversationally from an AI coding tool: it reads `SKILL.md` and the `templates/`/`resources/` packs to author the review, then calls this same reporting layer.",
  );
  lines.push("");
