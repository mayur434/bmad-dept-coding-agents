...
function writeDesignIndex(
  ...
  lines.push("");
  lines.push(`_Generated by BMAD Architecture Agent — headless CLI fallback._`);
  ...
  lines.push("");
  lines.push(
    "This is the headless CLI fallback — it emits standardized reporting plumbing only. For real ADR / HLD / LLD / API-contract / diagram / threat-model / data-model content, drive this agent conversationally from an AI coding tool: it reads `SKILL.md` and the `templates/`/`resources/` packs to author the artifacts, then calls this same reporting layer.",
  );
  lines.push("");
