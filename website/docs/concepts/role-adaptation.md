...
Every one of the eleven agents adapts its default mode, output shape, and recommended follow-ups to the **role** of the person driving the run. Role handling is a shared foundation (`skills/shared/role/`) consumed by every agent, and it is entirely opt-out (set `role: generic` to disable).
